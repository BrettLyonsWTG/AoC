...
{
    foreach (var n in ranges.Except([r])
        ...
        .Where(o => o.d == 1))
    {
        ...
        List<(int x, int y)> gaps = Enumerable.Range(0, Math.Min(Math.Abs(dx), Math.Abs(dy)) + 1)
            .Select(i => (x: start.x - i * vx, y: start.y + i * vy))
            .ToList();
        if (Debugger.IsAttached) PrintRanges(r.s, r.r, n.s, n.r, gaps);
        Console.WriteLine(new { r.s, gaps.Count });
    }
}