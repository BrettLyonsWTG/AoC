...
var endings = new (((int x, int y) p, char d) last, ((int x, int y) p, char d) scnd)[]
    {
        (((end.x + 1, end.y), '<'), ((end.x + 1, end.y - 1), 'v')),
        (((end.x + 1, end.y), '<'), ((end.x + 2, end.y), '<')),
        (((end.x + 1, end.y), '<'), ((end.x + 1, end.y + 1), '^')),
        (((end.x - 1, end.y), '>'), ((end.x - 1, end.y - 1), 'v')),
        (((end.x - 1, end.y), '>'), ((end.x - 2, end.y), '>')),
        (((end.x - 1, end.y), '>'), ((end.x - 1, end.y + 1), '^')),
        (((end.x, end.y + 1), '^'), ((end.x - 1, end.y + 1), '>')),
        (((end.x, end.y + 1), '^'), ((end.x, end.y + 2), '^')),
        (((end.x, end.y + 1), '^'), ((end.x + 1, end.y + 1), '<')),
        (((end.x, end.y - 1), 'v'), ((end.x - 1, end.y - 1), '>')),
        (((end.x, end.y - 1), 'v'), ((end.x, end.y - 2), 'v')),
        (((end.x, end.y - 1), 'v'), ((end.x + 1, end.y - 1), '<')),
    }.Where(e => 
        e.last.p.x >= 0 && e.scnd.p.x >= 0 && e.last.p.y >= 0 && e.scnd.p.y >= 0 &&
        e.last.p.x <= max.x && e.scnd.p.x <= max.x && e.last.p.y <= max.y && e.scnd.p.y <= max.x &&
        !walls.Contains(e.last.p) && !walls.Contains(e.scnd.p))
    .ToArray();

foreach (var path in endings)
{
    PrintTrack([path.scnd, path.last]);
}
