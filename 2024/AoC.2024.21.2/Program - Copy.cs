...
List<List<char>> GetNumPresses(char button, ref (int x, int y) pos)
{
    ...
    //var result = (next.x - pos.x, next.y - pos.y) switch
    //{
    //    ( > 0, _) => new string('>', next.x - pos.x) + new string('v', Math.Abs(next.y - pos.y)),
    //    ( < 0, _) when next != () => new string('<', next.x - pos.x) + new string('v', next.y - pos.y),
    //    ( < 0, < 0) => new string('^', Math.Abs(next.y - pos.y)) + new string('<', Math.Abs(next.x - pos.x)),
    //    ( < 0, > 0) => new string('<', Math.Abs(next.x - pos.x)) + new string('^', Math.Abs(next.y - pos.y)),
    //    _ => ""
    //};
    ...
    //var xpresses = (next.x - pos.x) switch
    //{
    //    0 => "",
    //    var x and > 0 => new string('>', x),
    //    var x and < 0 => new string('<', Math.Abs(x))
    //};
    //var ypresses = (next.y - pos.y) switch
    //{
    //    0 => "",
    //    var y and > 0 => new string('v', y),
    //    var y and < 0 => new string('^', Math.Abs(y))
    //};

    //var result =
    //    (pos.y == 3 && next.x == 0) ? ypresses + xpresses :
    //    (pos.x == 0 && next.y == 3) ? xpresses + ypresses :
    //    xpresses + ypresses;

    //pos = next;
    //return new List<List<char>>([[.. result.ToCharArray()]]);
}
